...
- **Phase 3 — live**: scenes & crossfades ✅, the appearance simulator ✅, external frame input (TiXL over TCP, any DDP sender — voxeled as a DDP Display) ✅, **inputs + merge** — Art-Net (with ArtPoll discovery), sACN, DDP, TCP and pages on the bus driving the piece simultaneously, per-pixel priority / HTP / LTP with timeouts and failover to the show, input maps for real controller wiring ✅; still ahead: dynamic scenes (live re-localization / MVR-xchange), DAW-style channels + modulators (LFOs / envelopes / audio), the live-frame VJ jack-in (NDI).
